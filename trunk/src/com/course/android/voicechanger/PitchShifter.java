...
/****************************************************************************
 * 
 * This code was converted to C# by Michael Knight madmik3 at gmail dot com.
...
public class PitchShifter {
	...
	public static void PitchShift(float pitchShift, long numSampsToProcess,
		...
		PitchShift(pitchShift, numSampsToProcess, (long) 256, (long) 10,
				sampleRate, indata);
		...
		for (i = 0; i < numSampsToProcess; i++) {
			...
			if (gRover >= fftFrameSize) {
				...
				for (k = 0; k <= fftFrameSize2; k++) {
					...
					gFFTworksp[(int) (2 * k + 1)] = (float) (magn * Math
							.sin(phase));
				}
				...
				for (k = 0; k < fftFrameSize; k++) {
					window = -.5
							* Math.cos(2.0 * Math.PI * (double) k
									/ (double) fftFrameSize) + .5;
					gOutputAccum[(int) k] += (float) (2.0 * window
							* gFFTworksp[(int) (2 * k)] / (fftFrameSize2 * osamp));
				}
			...
			for (k = 0, le = 2; k < max; k++) {
				...
				for (j = 0; j < le2; j += 2) {
					...
					for (i = j; i < 2 * fftFrameSize; i += le) {
						tr = fftBuffer[(int) (i + le2)] * ur
								- fftBuffer[(int) (i + le2 + 1)] * ui;
						ti = fftBuffer[(int) (i + le2)] * ui
								+ fftBuffer[(int) (i + le2 + 1)] * ur;
						fftBuffer[(int) (i + le2)] = fftBuffer[(int) i] - tr;
						fftBuffer[(int) (i + le2 + 1)] = fftBuffer[(int) (i + 1)]
								- ti;
						fftBuffer[(int) i] += tr;
